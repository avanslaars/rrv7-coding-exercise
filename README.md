...
This exercise is intended to be treated like a pair-programming session with you in the driver's seat.

You can ask questions, read docs, search the web, even use AI coding tools. We ask that all of these resources are used **on screen** so we can see your workflow and how you go about figuring things out and solving problems.

This project has a partially defined "Products" route at `app/routes/products.tsx`. The route reads data in the loader using a service function called `getProducts` which is defined in `app/services/product.ts`. Any associated types and the Zod schemas that the types are derived from are defined in `app/services/product.types.ts` The data it returns is handled via MSW and is all mock data. You shouldn't need to worry about the mocks, but if you are interested those are defined in `app/services/product.mocks.ts`.

You are being tasked with adding some functionality to this route. You probably won't be asked to complete every step below, so clarify which task to start with and after each step, you'll find out what step to implement next (if any).
...
The goal of this task is to create a second form on the page to add a new product to the list. It should accept a product name, price, and an optional description. The form should submit via the route's action and call the `createProduct` service function in `app/services/product.ts` to add the product.
...
There is a `getProductById` service function that will retrieve a single product using the product ID. Leverage this to create a _new route_ that shows the full product details, showing the name, price and description if available.
